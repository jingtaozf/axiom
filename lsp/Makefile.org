...
\verb|SET_REAL_MAXPAGE| which forces a call to \verb|init_shared_memory()|.
...
*** gmp wrappers patch
The file \verb|gmp_wrappers.h| has the declaration of the integer $j$
...
for the windows port. Apparently the \verb|$^| variable is not
...
gmp build. The fix seems to be to copy a new version of {\tt redc\_1.asm}
to {\tt gmp4/mpn/x86\_64/k8}. We have cached the correct {\tt redc\_1.asm}
in the zips subdirectory named {\tt gcl-2.6.12.gmp4.mpn.k8.fix}
...
\verb|cmpnew/gcl_collectfn| contains modifications to the common lisp compiler
