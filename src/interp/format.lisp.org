...
#+BEGIN_abstract
Note that this is a merger of my code for handling examples and Waldek's
code which fixes bug 7217 bad title generated in Axiom 3D output.
#+END_abstract
#+NAME: *
...
(DEFUN |matrix2String,outtranRow| (|x|)
  (if (symbolp |x|) 
    (concatenate 'string "MATRIX(" (symbol-name |x|) ")")
  (PROG ()
    (RETURN
     (|concat| (|lbrkSch|)
      (|tuple2String|
    ...
    (RETURN
     (COND
      ...
      ((AND (|isValidType| |m|) (CONSP |m|)
       ...
       (COND
        ((SETQ |x'| (|coerceInteractive| (mkObjWrap |x| |m|) |$OutputForm|))
         (|form2String1| (|objValUnwrap| |x'|)))
        (#1='T (|form2String1| |x|))))
      (#1# (|form2String1| |x|))))))
  ...
  (PROG (|$declVar| |rs| |ls| |whereBefore|)
    ...
    (RETURN
     (PROGN
      ...
      (COND ((AND (NE |$whereList| |whereBefore|) |$permitWhere|) |ls|)
