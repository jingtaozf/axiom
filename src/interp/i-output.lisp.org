...
** CHARYBDIS by Jonathan K. Millen
*** Introduction
...
the /display list/, is handed line by line to SCYLLA, a function
...
/display list/ to SCYLLA. CHARYBDIS enters the function APP to
...
1. The /origin/ is a distinguished location within a display,
...
2. The /width/ of a display is the number of columns from the
  extreme left of the display (the origin, in fact) to its extreme
  right.
3. The /superspan/ of a display is the number of lines it
  extends above the origin
4. The /subspan/ of a display is the number of lines it extends
  below the origin
...
1. a function whose four arguments are an expression, the two
  ...
  the display list)
2. a function of one argument whose value is the width of its
  argument
3. a function of one argument whose value is its superspan
4. a function of one argument whose value is its subspan.
...
sign. The plus sign is a character, /a fortiori/ an atom, and may
...
- - x - is the horizontal distance along the page at which to start
- - y - is some vertical hacking control
- - d - is the "layout" so far
  ...
  (prog (|negative| tmp1 tmp2 n)
    ...
    (return
      (cond
        ((STRINGP u)
         (cond
           ((or (eq u |$EmptyString|) (eql 0 (length u))) 0)
           ((and (eq (elt u 0) '%)
                 (or (char= (elt u 1) #\b)
                     (char= (elt u 1) #\d)))
            1)
