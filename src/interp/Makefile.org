...
the PROCLAIMS form. Under {\bf LISPFLAVOR=sbcl} the proclaims file is
...
\verb|(FTYPE (FUNCTION (*) ...))|, and ANSI has no \verb|(*)|
argument-list form, so SBCL rejects the file at the first proclaim
(observed, not assumed). Proclaims are compilation \emph{hints}; the
