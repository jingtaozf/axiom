...
of =$immediateDataSymbol= as their name. There are undoubtably still
...
(defun |mkAtreeNode| (x)
 (let (v)
  (setq v (make-array 5 :initial-element nil))
  (setelt v 0 x)
  v))
...
the lookup will fail. The =$envHashTable= was populated in
 ...
 (let (pl u)
   (cond
    ((setq u (|assoc| prop (setq pl (|getProplist| x e))))
     (setq e (|addBinding| x (remove (car u) pl :key #'car) e))
     e)
    (t e))))

#+end_src

*** fastSearchCurrentEnv
#+NAME: *
#+begin_src common-lisp :noweb yes
