...
=$PrimitiveDomainNames= is a list of domains that we need not cache.
...
- domainOrPackage -- ``domain'' or ``package'' which marks the kind
  of category object.
               ...
               (SEQ (EXIT (PROGN
                            (setq |OldLocals|
                                     (CONS (CONS |u| |count|)
                                      |OldLocals|))
                            (setq |count| (+ |count| 1))))))
             (setq |v| (make-array |count| :initial-element nil))
             (SETELT |v| 0 NIL)
             (SETELT |v| 1 |sigList|)
             (SETELT |v| 2 |attList|)
