...
;;; The q-accessors are GCL's fast car/cdr: (car (the cons x)), an
;;; UNCHECKED access that trusts x is a cons. GCL never verifies, so the
;;; interpreter is free to apply them to non-lists -- compColon, for one,
;;; reaches qcdr on the symbol S while compiling a (: S Type) declaration.
;;; SBCL cannot reproduce that: (the cons x) is a hard compile-time error
;;; when x is provably nil, (qListOrNil x) errors at run time when x is a
;;; symbol, and an unchecked (sb-ext:truly-the) car of a symbol segfaults.
;;; The only crash-free, error-free match for GCL's leniency is to treat a
;;; non-list as the empty list: qListOrNil returns its argument when it is
;;; a list (cons or nil) and nil otherwise, so (car (qListOrNil x)) yields
;;; nil for a non-list exactly where GCL returned a benign value. Every
;;; q-accessor below is phrased through it. This costs a listp test per
;;; access; correctness first, and the hot paths can move back to
;;; truly-the once the interpreter is proven never to pass a non-list.
(declaim (inline qListOrNil))
(defun qListOrNil (x) (if (listp x) x nil))
...
(defmacro qcaar (x)
 `(car (qListOrNil (car (qListOrNil ,x)))))

(defmacro qcadr (x)
 `(car (qListOrNil (cdr (qListOrNil ,x)))))

(defmacro qsecond (x)
 `(car (qListOrNil (cdr (qListOrNil ,x)))))

(defmacro qcdar (x)
 `(cdr (qListOrNil (car (qListOrNil ,x)))))

(defmacro qcddr (x)
 `(cdr (qListOrNil (cdr (qListOrNil ,x)))))

(defmacro qcaaar (x)
 `(car (qListOrNil (car (qListOrNil (car (qListOrNil ,x)))))))
(defmacro qcaadr (x)
 `(car (qListOrNil (car (qListOrNil (cdr (qListOrNil ,x)))))))
(defmacro qcadar (x)
 `(car (qListOrNil (cdr (qListOrNil (car (qListOrNil ,x)))))))
(defmacro qcaddr (x)
 `(car (qListOrNil (cdr (qListOrNil (cdr (qListOrNil ,x)))))))
(defmacro qthird (x)
 `(car (qListOrNil (cdr (qListOrNil (cdr (qListOrNil ,x)))))))
(defmacro qcdaar (x)
 `(cdr (qListOrNil (car (qListOrNil (car (qListOrNil ,x)))))))
(defmacro qcdadr (x)
 `(cdr (qListOrNil (car (qListOrNil (cdr (qListOrNil ,x)))))))
(defmacro qcddar (x)
 `(cdr (qListOrNil (cdr (qListOrNil (car (qListOrNil ,x)))))))
(defmacro qcdddr (x)
 `(cdr (qListOrNil (cdr (qListOrNil (cdr (qListOrNil ,x)))))))

(defmacro qcaaaar (x)
 `(car (qListOrNil (car (qListOrNil (car (qListOrNil (car (qListOrNil ,x)))))))))
(defmacro qcaaadr (x)
 `(car (qListOrNil (car (qListOrNil (car (qListOrNil (cdr (qListOrNil ,x)))))))))
(defmacro qcaadar (x)
 `(car (qListOrNil (car (qListOrNil (cdr (qListOrNil (car (qListOrNil ,x)))))))))
(defmacro qcaaddr (x)
 `(car (qListOrNil (car (qListOrNil (cdr (qListOrNil (cdr (qListOrNil ,x)))))))))
(defmacro qcadaar (x)
 `(car (qListOrNil (cdr (qListOrNil (car (qListOrNil (car (qListOrNil ,x)))))))))
(defmacro qcadadr (x)
 `(car (qListOrNil (cdr (qListOrNil (car (qListOrNil (cdr (qListOrNil ,x)))))))))
(defmacro qcaddar (x)
 `(car (qListOrNil (cdr (qListOrNil (cdr (qListOrNil (car (qListOrNil ,x)))))))))
(defmacro qcadddr (x)
 `(car (qListOrNil (cdr (qListOrNil (cdr (qListOrNil (cdr (qListOrNil ,x)))))))))
(defmacro qfourth (x)
 `(car (qListOrNil (cdr (qListOrNil (cdr (qListOrNil (cdr (qListOrNil ,x)))))))))
(defmacro qcdaaar (x)
 `(cdr (qListOrNil (car (qListOrNil (car (qListOrNil (car (qListOrNil ,x)))))))))
(defmacro qcdaadr (x)
 `(cdr (qListOrNil (car (qListOrNil (car (qListOrNil (cdr (qListOrNil ,x)))))))))
(defmacro qcdadar (x)
 `(cdr (qListOrNil (car (qListOrNil (cdr (qListOrNil (car (qListOrNil ,x)))))))))
(defmacro qcdaddr (x)
 `(cdr (qListOrNil (car (qListOrNil (cdr (qListOrNil (cdr (qListOrNil ,x)))))))))
(defmacro qcddaar (x)
 `(cdr (qListOrNil (cdr (qListOrNil (car (qListOrNil (car (qListOrNil ,x)))))))))
(defmacro qcddadr (x)
 `(cdr (qListOrNil (cdr (qListOrNil (car (qListOrNil (cdr (qListOrNil ,x)))))))))
(defmacro qcdddar (x)
 `(cdr (qListOrNil (cdr (qListOrNil (cdr (qListOrNil (car (qListOrNil ,x)))))))))
(defmacro qcddddr (x)
 `(cdr (qListOrNil (cdr (qListOrNil (cdr (qListOrNil (cdr (qListOrNil ,x)))))))))
...
(defmacro qrplaca (a b)
 `(rplaca (qListOrNil ,a) ,b))

(defmacro qrplacd (a b)
 `(rplacd (qListOrNil ,a) ,b))
...
(defun make-full-cvec (sint &optional (char #\space))
  (make-string sint
    :initial-element (if (integerp char) (code-char char) (character char))))

; 17.2 Accessing

(defun string2id-n (cvec sint)
  (if (or (< sint 1) (not (stringp cvec)))
      nil
...
;;; gcmsg toggles GCL's garbage-collection chatter flag and returns
;;; the previous setting. SBCL prints no per-GC message, so the
;;; portable branch just remembers the flag in a variable of our own:
;;; callers that save-and-restore the old value keep working, and
...
#-:GCL
(progn
 (defvar *gcmsg-flag* nil)
 (defun gcmsg (x)
   (prog1 *gcmsg-flag* (setq *gcmsg-flag* x))))

(defun reclaim () (gbc t))

;;; BPINAME's compiled branch asked GCL for the function's name; the
;;; ANSI route to the same answer is function-lambda-expression, whose
;;; third value is the name on every conforming host.
(defun BPINAME (func)
  (if (functionp func)
      (cond ((symbolp func) func)
            ...
            ((compiled-function-p func)
             #+:GCL (system:compiled-function-name func)
             #-:GCL (nth-value 2 (function-lambda-expression func)))
            ('t func))))
...
(defun QUOREM (i j r) ; never used, refed in parini.boot
  (multiple-value-bind (x y) (truncate i j)
   (rplaca (qListOrNil r) x) (rplacd (qListOrNil r) y)))
...
#+end_example
This whole C excursion exists because CLtL1 had no way to ask a hash
table for its test: the snippet casts the table object to a short
array and reads GCL's internal layout at offset 12. ANSI added
*hash-table-test*, which answers the same question by name on
every conforming host, so the portable branch is one case form with
no C at all. The GCL branch is kept verbatim behind a reader
conditional rather than switched to hash-table-test, because GCL
2.6.x's ANSI coverage is partial and the memory peek is known to
work there.
#+NAME: *
#+begin_src common-lisp :noweb yes
#+:GCL
...
(defun rpackfile (filespec)
  ...
  (if (string= (pathname-type (string filespec)) "nrlib")
  (let* ((base (pathname-name (string filespec)))
...
;;; ANSI's in-package is a macro taking an unevaluated string
;;; designator; the CLtL1 spelling (in-package 'boot) hands it the
;;; list (quote boot), which GCL tolerated and SBCL rejects as "not a
;;; string designator". The string spelling means the same on both.
(in-package "BOOT")
...
;;; These three macros expanded into (compiler-let nil ...) --
;;; compiler-let with an EMPTY binding list, doing nothing that progn
;;; does not. compiler-let is CLtL1, removed by X3J13 (its compile-
;;; time special bindings could not be given a portable meaning), and
;;; SBCL has no such operator. With no bindings the substitution is
;;; exact on every host, GCL included.
...
(defun -REDUCE (OP AXIS Y BODY SPL)
  (PROG (X G AUX EXIT VALUE PRESET CONSCODE RESETCODE)
   ...
   (SETQ PRESET (COND
      ...
      ((LIST 'SPADLET G Y)) ))
   (SETQ EXIT (COND
      ((SETQ X (ASSOC 'EXIT SPL))
         (SETQ SPL (remove 'EXIT SPL :key #'car)) (COND
...
(defmacro THETA (&rest LL)
  (let (U (L (copy-list LL)))
    (if (EQ (IFCAR L) '\,)  `(theta CONS . ,(CDR L))
        (progn
...
;;; GCL reports cumulative GC time via (system:gbc-time); SBCL keeps
;;; the same quantity in sb-ext:*gc-run-time*, in internal time units
;;; like elapsedUserTime above. A host with neither reports 0 -- the
;;; value only feeds the )set messages time display.
(defmacro |elapsedGcTime| ()
  #+:GCL '(system:gbc-time)
...
(defun |applyWithOutputToString| (func args)
  ;; returns the list of applying func to args and a string produced
...
;;
;; GCL-only, by reader conditional: embed/unembed and
;; system:universal-error-handler are AKCL mechanisms with no analogue
;; elsewhere, and the single-colon symbol cannot even be read on
;; another host. What this hook provides at run time -- routing Lisp
;; errors into the interpreter's $BreakMode protocol (trapNumerics,
;; letPrint2, nobreak/query/resume) -- is NOT yet provided on SBCL;
;; the equivalent would hang off sb-ext:*invoke-debugger-hook*. Until
;; that is written, interpreter-level error trapping is degraded on
;; SBCL: errors reach the host debugger/top level instead of the
;; break-mode catches. Deferred deliberately; revisit when the
...
(DEFUN FUNLOC (func &aux file)
  ...
  (if file (list (pathname-name (string file)) (pathname-type (string file)) func)
    nil))
...
;;; SBCL needs explicit defvar for top-level SETQ targets.
(defvar |/MAJOR-VERSION| nil)
(defvar /VERSION nil)
(defvar |$newConstructorList| nil)
(defvar |$createLocalLibDb| nil)
(defvar |$newComp| nil)
(defvar |$permitWhere| nil)
(defvar |$newSystem| nil)
(defvar |$bootStrapMode| nil)
(defvar |$devaluateList| nil)
(defvar |$doNotCompressHashTableIfTrue| nil)
(defvar |$mutableDomains| nil)
(defvar |$maxSignatureLineNumber| nil)
(defvar |$functionLocations| nil)
(defvar |$functorLocalParameters| nil)
(defvar |$insideCategoryPackageIfTrue| nil)
(defvar |$insideCompileBodyIfTrue| nil)
(defvar |$compilingInputFile| nil)
(defvar |$Slot1DataBase| nil)
(defvar |$ruleSetsInitialized| nil)
