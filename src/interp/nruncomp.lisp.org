...
;;; GCL implicitly globalises (declare (special x)); SBCL does not.
;;; These defvars ensure the NRT compiler variables have default
;;; bindings even when compDefineFunctor1 is not on the call stack.
(defvar |$NRTaddForm| nil)
(defvar |$NRTaddList| nil)
(defvar |$NRTattributeAlist| nil)
(defvar |$NRTbase| nil)
(defvar |$NRTdeltaLength| nil)
(defvar |$NRTdeltaList| nil)
(defvar |$NRTdeltaListComp| nil)
(defvar |$NRTderivedTargetIfTrue| nil)
(defvar |$NRTdomainFormList| nil)
(defvar |$NRTloadTimeAlist| nil)
(defvar |$NRTmakeCompactDirect| nil)
(defvar |$NRTslot| nil)
(defvar |$NRTslot1Info| nil)
(defvar |$NRTslot1PredicateList| nil)
(defvar |$NRTvec| nil)
(defvar |$template| nil)
(defvar |$domainShell| nil)
(defvar |$QuickCode| nil)
  ...
  (PROG ($GENNO |$catvecList| |$hasCategoryAlist| |$catNames| |$catsig|
    ...
    (RETURN
      (SEQ (PROGN
             ...
             (setq |domainShell|
                      (make-array (+ 6 |$NRTdeltaLength|) :initial-element nil))
             ...
             (setq |$maximalViews| NIL)
             (setq |$SetFunctions| (make-array (SIZE |domainShell|) :initial-element nil))
             (setq |$MissingFunctionInfo|
                      (make-array (SIZE |domainShell|) :initial-element nil))
             (setq |$catNames|
                      (CONS '$
                            (PROG (G166801)
                              (setq G166801 NIL)
             ...
             (DO ((G167008 (MAXINDEX |domainShell|))
               ...
               (SEQ (EXIT (PROGN
                            ...
                            (COND
                              ((BOOT-EQUAL |v| 'T) NIL)
                              ((NULL |v|) NIL)
                              ;; A vector slot can hold a non-list atom (a
                              ;; number such as 0); (CAR v) on it errors
                              ;; under SBCL where GCL's lenient CAR gave nil
                              ;; and the slot was skipped. Test (ATOM v)
                              ;; first so a non-cons slot is skipped without
                              ;; the CAR.
                              ((ATOM |v|) NIL)
                              ((ATOM (CAR |v|)) NIL)
                              ((ATOM |v|)
...
(DEFUN |NRTvectorCopy| (|cacheName| |domName| |deltaLength|)
  ...
  (make-array (+ 6 |deltaLength|) :initial-element nil))
