...
#+end_src
** Function getConstantFromDomain
[[getConstantFromDomain]] is used to look up the constants $0$ and $1$
from the given [[domainForm]].
1. if [[isPartialMode]] (see i-funsel.boot) returns true then the
...
2. they try to coerce t1 to t2 directly (tower coercion), and, if
...
3. the tower coercion looks whether there is any applicable local
...
4. if u=$fromCoerceable$, then TRUE or NIL
5. if the coercion succeeds, the coerced value (this may be NIL)
6. if the coercion fails, they throw to a catch point in
     coerceByFunction
