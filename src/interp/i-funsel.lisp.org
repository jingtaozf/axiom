...
\end{verbatim}
where:
- *n* is a penalty taking into account the number of coercions
  necessary to coerce the types of the given arguments to those of the
  signature under consideration.
- *res* is the codomain of the signature
- *hitListOfTarget* assigns a penalty between 1 and 1600 using
...
- *domainDepth* calculates the maximal depth of the type
- *finally* the preference order of PI, NNI, and DFLOAT as
