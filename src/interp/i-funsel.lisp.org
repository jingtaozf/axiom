...
(defun |getOpArgTypes,f| (x op)
 (let ( m (result x))
 (declare (special |$e|))
  (if (and (eqcar x '|FunctionCalled|) (not (eq op '|name|)))
...
where:
\begin{itemize}
\item *n* is a penalty taking into account the number of coercions
necessary to coerce the types of the given arguments to those of the
signature under consideration.
\item *res* is the codomain of the signature
\item *hitListOfTarget* assigns a penalty between 1 and 1600 using
...
\item *domainDepth* calculates the maximal depth of the type
\item *finally* the preference order of PI, NNI, and DFLOAT as
