...
(DEFUN |makeTemplate| (|vec|)
  (PROG (|newVec| |item| |sig| |op| |flag| |dcIndex| |code|
    ...
    (RETURN
      (SEQ (PROGN
             (setq |newVec| (make-array (SIZE |vec|) :initial-element nil))
...
(DEFUN |mkUniquePred| (|pred|) (|putPredHash| |pred|))
  ...
  (PROG (|n| |m| |newVec|)
    (RETURN
      (SEQ (PROGN
             ...
             (setq |newVec| (make-array |m| :initial-element nil))
  ...
  (declare (special |$predVector| |$predVectorFrontier| |$consDB| |$predHash|))
    (RETURN
      (SEQ (PROGN
             ...
             (setq |$predVector| (make-array 100 :initial-element nil))
...
(DEFUN |list2LongerVec| (|u| |n|)
  (PROG (|vec|)
    (RETURN
      (SEQ (PROGN
             (setq |vec| (make-array (QUOTIENT (TIMES 7 |n|) 5) :initial-element nil))
