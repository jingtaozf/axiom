             ...
             (DO ((G3179 (NREVERSE (SORTBY |sortFn| |reportList|))
                 ...
                 ((OR (ATOM G3179)
                      ...
                      (PROGN
                        ...
                        NIL))
                  NIL)
;;; One stray close paren removed from the line below (GCL's reader
;;; discarded it silently; SBCL aborts the load). A character-level
;;; balance scan of the whole tangled file confirms this was the only
;;; surplus and the file is balanced after the fix.
               (SEQ (EXIT (PROGN
                            (|sayBrightlyNT|
    ...
    (RETURN
      (PROGN
        ...
        (COND
          ...
          ('T
           (CONS (|fillerSpaces| (- |maxWidth| SIZE) '| |)
                 (CONS |x| NIL))))))))
