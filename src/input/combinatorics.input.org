...
source file =combfunc.spad= contains a few functions for
dealing with factorials, and permuations themselves and permutation
groups are provided by =perm.spad= and =permgrp.spad=.
...
#+end_src
\begin{description}
  \item=powerSet= Given a set, produces its power set: the set of all
    subsets.  For example:
#+NAME: input
#+begin_src common-lisp :noweb yes
--S 1 of 21
...
\item =choose= 
The function =choose(S,n)= lists all subsets of $S$
...
\begin{description}
\item[=listPermutations=] Given a list containing any elements, 
...
1. find the largest $j$ such that $p.j>p.k$
1. swap $p.j$ and $p.k$
1. reverse that part of the list from indices $k+1$ to $n$.
...
\item[=randomPermutation=] This uses Knuth's shuffle algorithm; for 
...
A /derangement/ is a permutation where none of the elements are in their
...
\begin{description}
\item[=derangements(n)=] produces the number of derangements of $n$ 
...
\item[=listDerangements(L)=] lists all the derangements of a list, by
...
\item[=countDerangements(L)=] This produces the number of derangements 
of any list.  If there are $k$ distinct elements of $L$ have multiplicities
$a_1,a_2,...,a_k$, then the number of derangements was first published 
by Major Percy MacMahon in ``Combinbatory Analysis'' (1915):

Let $x_1,x_2,...,x_k$ be variables and put $S=x_1+x_2+\cdots+x_k$.  Then the
...
\item[=randomDerangement(L)=] This is a probabilistic function: 
...
A /partition/ of a set $S$ is a splitting of $S$ into a set of 
...
called the /Bell number/, denoted $B_n$.  Using Knuth's bracketing 
...
\begin{description}
\item[=listPartitions=] This function lists all partitions as codes, 
...
\item[=setPartitions=] returns the partitions of a set, by mapping the
...
\item[=listSizePartitions(n,k)=] is a slight variant of
=listPartitions= and lists only those codes whose maximum value is $k$.
...
\item[=setSizePartitions(S,k)=] turns the list of codes into 
...
\item[=bell(n)=] returns the $n$-th Bell number
...
\item[=touchard(n,x)=] returns the $n$-th Touchard polynomial:
...
<<countDerangements>>
<<randomDerangement>>
<<listStringPermutations>>
<<stringFixed?>>
<<listStringDerangements>>
<<listPartitions>>
<<setPartitions>>
<<listSizePartitions>>
<<setSizePartitions>>
<<bell>>
<<touchard>>
<<randomStringPermutation>>
<<randomStringDerangement>>
<<input>>
