...
\int{sech^3~{ax}}~dx$}
$$\int{sech^3~{ax}}=
\frac{sech~{ax}~\tanh{ax}}{2a}+\frac{1}{2a}\tan^{-1}~sech~{ax}
$$
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
\int{sech^n~{ax}~{\tanh{ax}}}~dx$}
$$\int{sech~^n{ax}~{\tanh{ax}}}=
-\frac{sech~^{n}{ax}}{na}
$$
#+NAME: *
#+begin_src common-lisp :noweb yes
...
\int{\frac{dx}{q+p~sech~{ax}}}~dx$}
$$\int{\frac{1}{q+p~sech~{ax}}}=
...
\int{sech^n~{ax}}~dx$}
$$\int{sech^n~{ax}}=
\frac{sech^{n-2}~{ax}~\tanh{ax}}{a(n-1)}
+\frac{n-2}{n-1}\int{sech^{n-2}~{ax}}
$$
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
