...
1. Copy the quaternion source code and edit it to use $\sigma_i$ as
...
1. Use the $\sigma$ basis above with the existing package but define a
...
used by Gsponer and others. A biquat $[*a*, *b*, *c*, *d*]$
has scalar component $*a*$ which may have itself an imaginary component.
The component $[*0*, *b*, *c*, *d*]$ is its vector 
...
1. How would one perform a proof such as the above within the
allowed operations of Axiom?
1. How can Axiom be called a symbolic algebra system if basic notions
...
Recall that Axiom does things in a fundamentally /algebraic/ rather
than /symbolic/ way. So although the pattern matching approach
