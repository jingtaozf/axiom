...
If $A$ is a given square matrix $n\ x\ n$ and $I_n$ is the 
$n\ x\ n$ identity matrix, then the characteristic polynomial
of $A$ is defined at 
$$p(\lambda) = det(\lambda{}I_n - A)$$
where =det= is the determinant function. The Cayley-Hamilton
