...
2. The equilibrium point is unstable if the linearised system is
strictly unstable.
3. If the linearised system is marginally stable, then we cannot
...
$V(\overline{x}) = 0$. Then,
4. $V$ is positive definite on $S$ if $V(x) > 0$, for $x \ne \overline{x}$
5. $V$ is negative definite on $S$ if $V(x) < 0$, for $x \ne \overline{x}$
6. If the equalities are not strict, then $V$ is considered positive and
...
properties:
7. $V(\overline{x}) = 0$
8. $V(x)$ is positive definite for $x \ne \overline{x}$
9. $\dot{V}(x)$ is negative semi-definite along trajectories of
$\dot{x} = f(x)$
