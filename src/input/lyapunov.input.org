...
2. $V$ is negative definite on $S$ if $V(x) < 0$, for $x \ne \overline{x}$
3. If the equalities are not strict, then $V$ is considered positive and
...
properties:
1. $V(\overline{x}) = 0$
2. $V(x)$ is positive definite for $x \ne \overline{x}$
3. $\dot{V}(x)$ is negative semi-definite along trajectories of
  $\dot{x} = f(x)$
