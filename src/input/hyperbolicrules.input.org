...
#+TITLE: hyperbolicrules.input
#+AUTHOR: Timothy Daly
#+BEGIN_abstract
#+END_abstract
** Definition of Hyperbolic Functions
*** 8.1 Hyperbolic sine of x
...
#+end_src
\subsection{8.11 $cosh^2(x)-~sinh^2(x)$}
\[cosh^2(x)-~sinh^2(x) == 1\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
#+end_src
\subsection{8.12 $sech^2(x)+~tanh^2(x)$}
\[sech^2(x)+~tanh^2(x) == 1\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
#+end_src
\subsection{8.13 $coth^2(x)-~csch^2(x)$}
\[coth^2(x)-~csch^2(x) == 1\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
#+end_src
\subsection{8.22 $tanh(x \pm y)$}
\[tanh(x \pm y) == \frac{tanh(x)\pm~tanh(y)}{1\pm~tanh(x)~tanh(y)}\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
#+end_src
** Half Angle Formulas
\subsection{8.27 $sinh\left(\frac{x}{2}\right)$}
\[sinh\left(\frac{x}{2}\right) == \pm\sqrt{\frac{cosh(x) - 1}{2}} [+~if\ \x > 0, -if\ \x < 0]\]
...
#+end_src
\subsection{8.28 $cosh\left(\frac{x}{2}\right)$}
...
#+end_src
\subsection{8.29 $tanh\left(\frac{x}{2}\right)$}
\[tanh\left(\frac{x}{2}\right) == \pm\sqrt{\frac{cosh(x) - 1}{cosh(x) + 1}} [+~if\ \x > 0, -if\ \x < 0]\]
...
#+end_src
\subsection{8.35 $tanh(4x)$}
\[tanh(4x) == \frac{4~tanh(x)+4~tanh^3(x)}{1+6~tanh^2(x)+tanh^4(x)}\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
#+end_src
** Sum, Difference, and Product of Hyperbolic Products
\subsection{8.42 $sinh(x)+sinh(y)$}
...
\subsection{8.55 $sinh^{-1}(x)$}
\[asinh(x) == \log(x+\sqrt{x^2+1}) if\ \ -\infty < x < \infty\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
#+end_src
\subsection{8.56 $cosh^{-1}(x)$}
\[acosh(x) == \log(x+\sqrt{x^2-1}) if\ \ x \ge 1\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
\subsection{8.57 $tanh^{-1}(x)$}
\[atanh(x) == \frac{1}{2}\log(\frac{1+x}{1-x}) if\ \ -1 < x < 1\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
\subsection{8.58 $coth^{-1}(x)$}
\[acoth(x) == \frac{1}{2}\log(\frac{x+1}{x-1}) if\ \ x>1 or x<-1\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
\subsection{8.59 $sech^{-1}(x)$}
\[asech(x) == \log(\frac{1}{x}+\sqrt{\frac{1}{x^2}-1}) if\ \ 0 < x \le 1\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
#+end_src
\subsection{8.60 $csch^{-1}(x)$}
\[acsch(x) == \log(\frac{1}{x}+\sqrt{\frac{1}{x^2}+1}) if\ \ x \ne 0\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
\subsection{8.86 $sinh(x+2k\pi i)$}
...
#+end_src
\subsection{8.87 $cosh(x+2k\pi i)$} 
...
#+end_src
\subsection{8.88 $tanh(x+2k\pi i)$} 
\[tanh(x+k\pi i) == tanh(x)\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
#+end_src
\subsection{8.89 $csch(x+2k\pi i)$} 
...
#+end_src
\subsection{8.90 $sech(x+2k\pi i)$} 
...
#+end_src
\subsection{8.91 $coth(x+2k\pi i)$} 
\[coth(x+k\pi i) == coth(x)\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
** Inverse Hyperbolic and Inverse Trig Functions
...
#+begin_example
rootrule4a:RewriteRule(INT,COMPLEX(INT),EXPR(COMPLEX(INT))):=
  rule(sqrt(p^2-q^2)==sqrt(p-q)*sqrt(q+p))  schaum17.input.pamphlet:
tanrule2:RewriteRule(INT,COMPLEX(INT),EXPR(COMPLEX(INT))):=
  rule(tan(a) == sin(a)/cos(a))  schaum17.input.pamphlet:
#+end_example
