...
Maxima gets
$$x=\%c \%e^{\displaystyle
\frac{x asinh\left(\frac{y}{x}\right)}{\vert x\vert}}$$
which does not simplify to 0 on substitution.
...
Maxima gets
$$x=\%c \%e^{\displaystyle
-\frac{x asinh\left(\frac{y}{x}\right)}{a\vert x\vert}}$$
which does not simplify to 0 on substitution.
...
Mathematics get:
$$2e^{-e^{-x} C[1]+e^{-x}ExpIntegralEi[x]}x$$
which does not simplify to 0 on substitution.
#+NAME: *
#+begin_src common-lisp :noweb yes
--S 45 of 126
...
Maple gets:
$$\arctan\left(\frac{2x~\_C1}{1+x^2~\_C1^2} , -\frac{-1+x^2~\_C1^2}{1+x^2~\_C1^2}\right)x$$
which, on substitution, simplifies to 0.
...
Maple gives
$$\frac{e^x}
{-e^x-e^{(-1)}Ei(1,-x-1)x-e^{(-1)}Ei(1,-x-1)+x~\_C1+~\_C1}$$
which, on substitution, simplifies to 0.

Mathematica gives
$$-\frac{e^{1+x}}{e^{1+x}-eC[1]-exC[1]-ExpIntegralEi(1+x)-
xExpIntegralEi(1+x)}$$
#+NAME: *
#+begin_src common-lisp :noweb yes
--S 63 of 126
...
Maple gives
$$\left(Ei\left(1,\frac{1}{x}\right)+~\_C1\right)e^{(\frac{1}{x})}$$
which simplifies to 0 on substitution.

Mathematica gets:
$$e^{1/x}C[1]-e^{1/x}ExpIntegralEi\left(-\frac{1}{x}\right)$$
which simplifies to 0 on substitution.
#+NAME: *
#+begin_src common-lisp :noweb yes
--S 75 of 126
...
#+end_src
Maxima gets
$$(asinh(x)+\%c)\%e^{-\frac{\displaystyle\log(x^2+1)}{\displaystyle 2}}$$
which when substituted, does not simplify to 0.

Maple gets
$$\frac{arcsinh(x)+~\_C1}{\sqrt{x^2+1}}$$
which when substituted, simplifies to 0.

Mathematica gets
$$\frac{arcsinh(x)}{\sqrt{1+x^2}}+\frac{C[1]}{\sqrt{1+x^2}}$$
