...
1. Calculate the point (x, y) = k * G, using scalar multiplication.
1. Find r = x mod n. If r = 0, return to step 1.
1. Find s = (z + r * d) / k mod n. If s = 0, return to step 1.
1. The signature is the pair (r, s)
...
1. Calculate $u = z * w mod n$
1. Calculate $v = r * w mod n$
1. Calculate the point $(x, y) = uG + vQ$
1. Verify that $r = x mod n$. The signature is invalid if it is not.
