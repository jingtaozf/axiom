...
--)r "/spad/graphics/oldInput/curl"
rc := range curl1
--draw2D(rc.xRange,rc.yRange,sin(t)*sin(2*t)*sin(3*t),_
--       sin(4*t)*sin(5*t)*sin(6*t),0..2*pi())
draw(curve(sin(t)*sin(2*t)*sin(3*t),sin(4*t)*sin(5*t)*sin(6*t)),_
     t=0..2*pi(),[ranges([rc.0,rc.1])])
curl2:=getGraph(c2:=%,1)
