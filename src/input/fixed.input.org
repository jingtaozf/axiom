...
--R      declaration:
--R
--I   SAEa := SimpleAlgebraicExtension(Fraction Integer,...
--R   a : SAEa := a
--R
--R   (1)  a
...
One cannot simply parametrize reducible curves, it seems.
...
)lisp (format ofile "   rightProductBy(n:Integer):Mapping(Integer,Integer) ==")
)lisp (format ofile "     #1 * n~%")
)lisp (close ofile)
...
There is a bug in the function =delete_!=
for the type List ; this function,
has two arguments, a list L and an integer n : =delete_!(L, n)= 
