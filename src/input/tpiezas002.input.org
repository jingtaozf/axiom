...
7. $x^2+y^2=z^2+nt^k$
8. $x^2+y^2=mz^2+nt^2$
9. $c_1(x^2+ny^2)=c_2(z^2+nt^2)$
10. $mx^2+ny^2=mz^2+nt^2$
...
*Theorem*: For primitive triples $a^2+b^2=c^2$,\\
...
*Theorem*: If $a^2+b^2=c^2$ then $(a+2b+2c)^2+(2a+b+2c)^2=(2a+2b+3c)^2$
...
#+end_src
*Theorem*: All odd numbers and multiples of 4 appear in a primitive
Pythagorean triple
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
#+end_src
*** Piezas
*Theorem*: Let $F:=(p^2+q^2)(r^2+s^2)$. Then $F^k$ (for $k>0$) is
...
#+end_src
*** H. Mathieu
\[(q^2(p^2-2))^2 + (2q^2)^3 = (pq)^4 {\rm\ if\ }q^2-2q^2=1\]
\[((p^4-p^2)/2)^2 + p^6 = (pq)^4 {\rm\ if\ }p^2-2q^2=-1\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
#+end_src
*** Piezas
\[(4q^2d^4(p^2-2))^2+(4*q^2d^3)^3=(2pqd^2)^4 {\rm\ if\ }p^2-dq^2=1\]
\[(4*p^2*d^3(p^2-1))^2+(2*p*d)^6=(2*p*q*d^2)^4 {\rm\ if\ }p^2-dq^2=-1\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
#+end_src
*** K. Brown
\[p^4 + (q^2-1)^3 = (q^3+3q)^2 {\rm\ if\ }p^2-3q^2=1\]
#+NAME: *
#+begin_src common-lisp :noweb yes
)clear all
...
*http://sites.google.com/site/tpiezas/002*
\end{thebibliography}
\end{document}
