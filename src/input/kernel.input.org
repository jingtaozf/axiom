...
=argument= gives you a list of arguments of the kernel
...
There is also =tower=, which gives all the kernels, both top 
level and nested.
...
Compared to =kernels=, we also get 'x'

Pattern matching is implemented on top of =numer=, =denom=,
=isPlus=, =isTimes=, =isExpt=, and kernels handling.
For example
...
different from the printed form. Not that =isPlus= essentially 
gives you a list of monomials. =isTimes= gives factors of a
single monomial.
