...
DIRS is a list of directories with the suffix *dir*.
...
#+end_src
*** The share directory
The *share* directory files that are shared by all version of the
system.
...
#+end_src
*** The interp directory
Once *bootsys* exists we need to build *depsys*
and *interpsys*. Since these two images share a lot of
...
#+end_src
*** The algebra directory
The algebra directory contains code written in Axiom's computer
algebra language called *spad*. There are two compilers for
this language, the spad compiler and the *Aldor*[cite:@1] compiler.
...
The second call will execute the *all* stanza. This stanza walks 
...
\bibitem{1} Watt, Stephen, The Aldor compiler, *www.aldor.org*
