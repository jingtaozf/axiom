...
superfluous to discuss the problem at length... Further, the dignity of
...
\begin{flushright}
Karl Friedrich Gauss, /Disquisitiones Arithmeticae/, 1801 (translated
from [Knu98])
\end{flushright}
\end{quote}
** Introduction
...
proved what is referred as /Fermat's Little Theorem/ stating that for
any prime number $p$, and any number $a$ not divisible by $p$,
$a^{p-1} = 1 (mod p)$. Although the converse of this theorem does not hold
(and in fact fails spectacularly for /Carmichael numbers/, this result
...
polynomial-time on almost all inputs (/all/ inputs under a widely
...
*Lemma 3.1* 
/Let p and r be prime numbers, $p \ne r$./
1. {\sl The multiplicative group of any field $F_{p^t}$ for $t > 0$, denoted
by $F_{p^t}^*$ is cyclic.}
2. {\sl Let $f(x)$ be a polynomial with integral coefficients. Then
$$f(x)^p == f(x^p)(mod\ p)$$}
3. {\sl Let h(x) be any factor of $x^r - 1$. Let $m == m_r(mod\ r)$. Then
$$x^m == x^{m_r}\ (mod\ h(x))$$}
4. {\sl Let $o_r(p)$ be the order of $p\ module\ r$. Then in
$F_p$, ${{x^r - 1}\over{x-1}}$ factorises into irreducible polynomials
each of degree $o_r(p)$.}

/Proof/
5. See, e.g., [LN86]
6. Let $f(x) = a_0+a_1x+...+a_dx^d$. The coefficient of $x^i$ in
$f(x)^p$ is
$$\Sigma_{{i_0+...+i_d=p}\over{i_1+2i_2+...+di_d=i}}
a_0^{i_0} ... a_d^{i_d} {{p!}\over{i_0! ... i_d!}}$$
...
8. Let $d = o_r(p)$ and $Q_r(x) = {{x^r-1}\over{x-1}}$.
...
*Lemma 3.2* {\sl [Fou85, BH96] Let $P(n)$ denote the greatest prime
...
*Lemma 3.3*
/[Apo97] Let $\pi(n)$ be the number of primes $\le n$. Then for $n \ge 1$:/
$${{n}\over{6\ log\ n}} \le \pi(n) \le {{8n}\over{log\ n}}$$
** The Algorithm
\hrule
Input: integer $n > 1$
9. if ($n$ is of the form $a^b$, $b>1$) output COMPOSITE;
10. $r = 2$;
11. while $(r < n)$ \{
12. \ \ \ \ if ($gcd(n,r) \ne 1$) output COMPOSITE;
13. \ \ \ \ if ($r$ is prime)
14. \ \ \ \ \ \ \ \ let $q$ be the largest prime factor of $r - 1$;
15. \ \ \ \ \ \ \ \ if ($q \ge 4\sqrt{r}\ log\ n$) and ($n^{{r-1}\over{q}} !== 1 (mod\ r)$)
16. \ \ \ \ \ \ \ \ \ \ \ \ break;
17. \ \ \ \ $r \leftarrow r + 1$;
18. \}
19. for $a = 1$ to $2\sqrt{r}\ log\ n$
20. \ \ \ \ if ($(x-a)^n !== (x^n - a)(mod\ x^r-1,n)$) output COMPOSITE;
21. output PRIME;
...
of the *while* loop after which such an $r$ is found.
...
$r$'s (lets call them /special/ primes) between $c_1(log\ n)^6$ and
$c_2(log\ n)^6$ such that 
$P(r-1) > (c_2(log\ n)^6)^{{2}\over{3}} > r^{{2}\over{3}}$
is (for large enough $n$)
| \ \ \ \ \ \ \ \ | $\ge$ | No of special primes in $[1... c_2(log\ n)^6]-$ |
|  |  | No of primes in $[1... c_1(log\ n)^6]$ |
...
/Proof/. The *while* loop cannot return COMPOSITE since
$gcd(n,r) = 1$ for all $r \le c_2(log\ n)^6$, where $c_2$ is as in
Lemma 4.2. By Lemma 3.1 (fact 2), the *for* loop also cannot
...
our algorithm. The significance of the $r$ found by the *while* loop
...
The set of /l/ binomials form a large cyclic group in this field:
...
$F_p[x]/(h(x)). The =while= loop ensures that once it halts the final 
$r$ is such that $r > q > 4\sqrt(r)\ log\ n > l$. Also step 4 of the
algorithm checks =gcd= of $r$ and $n$. If any of the $a$'s are
