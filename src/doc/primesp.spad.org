...
by $F_{p^t}^*$ is cyclic.}
1. {\sl Let $f(x)$ be a polynomial with integral coefficients. Then
$$f(x)^p == f(x^p)(mod\ p)$$}
1. {\sl Let h(x) be any factor of $x^r - 1$. Let $m == m_r(mod\ r)$. Then
$$x^m == x^{m_r}\ (mod\ h(x))$$}
1. {\sl Let $o_r(p)$ be the order of $p\ module\ r$. Then in
$F_p$, ${{x^r - 1}\over{x-1}}$ factorises into irreducible polynomials
each of degree $o_r(p)$.}

/Proof/
1. See, e.g., [LN86]
1. Let $f(x) = a_0+a_1x+...+a_dx^d$. The coefficient of $x^i$ in
$f(x)^p$ is
...
1. Let $d = o_r(p)$ and $Q_r(x) = {{x^r-1}\over{x-1}}$.
...
1. \ \ \ \ if ($r$ is prime)
1. \ \ \ \ \ \ \ \ let $q$ be the largest prime factor of $r - 1$;
1. \ \ \ \ \ \ \ \ if ($q \ge 4\sqrt{r}\ log\ n$) and ($n^{{r-1}\over{q}} !== 1 (mod\ r)$)
1. \ \ \ \ \ \ \ \ \ \ \ \ break;
1. \ \ \ \ $r \leftarrow r + 1$;
1. \}
1. for $a = 1$ to $2\sqrt{r}\ log\ n$
1. \ \ \ \ if ($(x-a)^n !== (x^n - a)(mod\ x^r-1,n)$) output COMPOSITE;
1. output PRIME;
