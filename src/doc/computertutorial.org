...
- Printing is done using the *output*.  For example:\\
\begin{verbatim}
output "this is a number: "
\end{verbatim}

- Printing several items on the same line can by done with the
...
- Sequences of statements may be surrounded by braces. For example:
\begin{small}
\begin{verbatim}
{
  a := 3; 
...
- There is no =main= (unless you want one).
...
- domains and categories are similar to the C++ concept of
...
- In contrast to C and C++, which are never indentation sensitive,
...
--- see [[#asugManifest][asugManifest]]) we see the following domain
definition:
...
[[#asugManifest][asugManifest]]).  *BasicType* has the following
definition:
...
[[#axlcatFigure][axlcatFigure]] on pageref{axlcatFigure}).  So we could satisfy
