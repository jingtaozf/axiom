...
Under {\bf LISPFLAVOR=sbcl} this target changes meaning. There is no
...
host, namely \verb|${OBJ}/${SYS}/bin/lisp| (the {\bf LISPSYS} of
...
\verb|(tangle "foo.pamphlet" "*" "foo.lisp")| into it. A bare SBCL
...
The {\bf LISPFLAVOR} variable is that selector. It defaults to
{\bf gcl}, which leaves every recipe exactly as it always was. Saying
#+begin_example
make LISPFLAVOR=sbcl
#+end_example
selects the system SBCL instead. Two things change under sbcl. First,
the compiled-object extension {\bf O} becomes {\bf fasl}: SBCL's
...
\verb|${O}| follows automatically. Second, the {\bf lspdir} target
...
extensions are untouched: {\bf LISP} stays {\bf lsp} because the
