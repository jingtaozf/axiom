...
1. the symbol $*$ is a binary operation on the domain belonging to
this category
2. the identifier 1 denotes a constant object of a domain belonging
...
- ``multiplication'' of two objects in =IntMonoid= is the value
obtained by adding their respective underlying values (returned by the
$*$ operator);
- the =Integer= constant 0 is the underlying value of the unit
of =IntMonoid=
...
3. open the index.kaf file
4. (read) the integer
5. (seek) to the integer offset from the beginning of the file
6. (read) the index of triples
7. find the keyword (e.g. ``operationAlist'') triple
8. select the third element, an integer
9. (seek) to the integer offset from the beginning of the file
10. (read) the ``operationAlist''
...
- slot 0 holds the canonical category form of the expression whose
...
- slot 2 holds a list of attributes and the condition under which
they hold
- slot 3 always contains the form '(category)'. It serves as a runtime
type checking tag
...
- slot 5 holds the list of all domain forms mentioned in the exported
signatures
- each of the slots 6 and onwards holds either runtime information about
...
This simple command:
11. Invokes the Spad compiler and produces Lisp output.
12. Calls the Lisp compiler if the compilation was successful.
13. Uses the =)library= command to tell Axiom about
...
takes 3 arguments
14. the *form* which is specific to the function
15. the *mode* a |Join|, which is a set of categories and domains
16. the *env* which is a list of functions and their modemaps
...
17. the *form* which is an def specifying the domain to define.
18. the *mode* a |Join|, which is a set of categories and domains
19. the *env* which is a list of functions and their modemaps
...
20. the *form* which is an |add| specifying the domain
to extend and a set of functions to be added
21. the *mode* a |Join|, which is a set of categories and domains
22. the *env* which is a list of functions and their modemaps
...
23. the *compiled capsule* which is a progn form which returns
the domain
24. the *mode* from the input argument
25. the *env* prepended with the signatures of the functions
...
- *must* means that if it is not present in the token stream,
it is a syntax error.
- *optional* means that if it is present in the token stream,
that is a good thing, otherwise don't worry (like [ foo ] in BNF notation).
- *action* is something we do as a consequence of successful
parsing; it is inserted at the end of the conjunction of requirements 
for a successful parse, and so should return T.
- *sequence* consists of a head, which if recognized implies that the
