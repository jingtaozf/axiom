...
this category
1. the identifier 1 denotes a constant object of a domain belonging
...
1. (read) the integer
1. (seek) to the integer offset from the beginning of the file
1. (read) the index of triples
1. find the keyword (e.g. ``operationAlist'') triple
1. select the third element, an integer
1. (seek) to the integer offset from the beginning of the file
1. (read) the ``operationAlist''
...
1. Calls the Lisp compiler if the compilation was successful.
1. Uses the =)library= command to tell Axiom about
...
1. the *mode* a |Join|, which is a set of categories and domains
1. the *env* which is a list of functions and their modemaps
...
1. the *form* which is an |add| specifying the domain
...
1. the *compiled capsule* which is a progn form which returns
the domain
1. the *mode* from the input argument
1. the *env* prepended with the signatures of the functions
