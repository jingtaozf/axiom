...
#S(FN NAME BOOT::|Equation| DEF DEFUN VALUE-TYPE T FUN-VALUES
      ...
      RETURN-TYPE NIL ARG-TYPES (T) NO-EMIT NIL MACROS
      (VMLISP:HREM PROG1 VMLISP:HGET BOOT::LETT COND RETURN)) ))#+end_example
...
3. Uses the =)library= command to tell Axiom about
...
** Operator Precedence Table Initialization
#+begin_example
...
(")abbrev domain EQ Equation")
\end{verbatim
and the output is a *List (Cons NNI String)* containing
...
- Add open paren to beginning of following line if following line's
...
- If the entire line consists of the single keyword then or else,
  leave it alone."
...
3. the *env* prepended with the signatures of the functions
