...
** The Original Plan
\begin{center}
The Original Hypertex Plan (courtesy of Scott Morrison)\\
[[ps/hypertexplan.eps]]
\end{center}

** External Variables
Not mentioned elsewhere, 
- the HTPATH shell variable, if set, is used
to resolve page path names.
- the HTASCII shell variable, if set, is used to choose between
ascii and the IBM Code Page 850 character set.
See initScanner [[#initScanner][initScanner]] on page~[[#initScanner][initScanner]]
- the XENVIRONMENT shell variable, if set is used to find the X
database to merge, otherwise it uses .Xdefaults from the HOME directory.
See [[#mergeDatabases][mergeDatabases]] on page~[[#mergeDatabases][mergeDatabases]]
- NOFREE shell variable is supposed to turn off freeing memory.
See [[#freePage][freePage]] on page~[[#freePage][freePage]]
- SPADNUM shell variable is the number of the spad communication socket.
See [[#sendPile][sendPile]] on page~[[#sendPile][sendPile]]


The Axiom user properties in =$HOME/.Xdefaults.= can contain
...
#+NAME: include/all-hyper-proto.h1
#+begin_src common-lisp :noweb yes
...
#+end_src
** Shared Code for Hash Table Handling
*** halloc
...
#+end_src
** Shared Code for Error Handling
*** jump
#+INDEX: shared!jump
#+INDEX: jump shared
#+NAME: jump
...
** Shared Code for Lexical Analyzer
...
- getToken() -- sets the ``token'' variable to be the next token
in the current input stream 
- saveScannerState()  -- save the current state of scanner so
...
stored in the =$AXIOM/doc= subdirectory and there is
...
The =hypertex= function looks in =$AXIOM/doc= by
...
;; This file is generated by GNU cflow 1.3. -*- cflow -*-
   ...
   73 {   4}   | |   +-halloc() <char *halloc () line:1355> [see 11]
   74 {   4}   | |   +-strlen()
   75 {   4}   | |   \-strcpy()
...
To rebuild the hypertex database changes to the =$AXIOM/doc= 
...
1. If the user specifies a pathname, then it is the path name that
is used. If the user does not specify a path name, then possible
paths are found as follows:
  - If the user has an environment variable HTPATH set, the
...
This source file used to rely on =<regexp.h>= which was
...
namely through =<regex.h>=.  That is what we use now.  Its
...
#+end_src
** Managing the HyperDoc group stack
*** popGroupStack
...
#+end_src
** Handle input, output, and Axiom communication
*** makeRecord
...
(with =\axiomOp{key}= ps and grep) the
...
#+end_src
** X Window window initialization code
...
#+end_src
** Handling user page interaction
*** fillBox
...
#+end_src
** Handle InputString, SimpleBox, RadioBox input
...
#+end_src
** parsing routines for node types
*** parseIfcond
...
#+end_src
*** parseValue1
This parseValue routine accepts an empty ={}= but makes it a zero 
instead of a one. Thus =\indent{}= is equivelant to =\indent{0}=.
...
#+end_src
** Scrollbar handling routines
The scrollbar is displayed on the side of the HyperDoc display, if needed.
It is composed of four windows
- fScrollUpWindow -- the arrowed box at the top of the scrollbar.
Scrolls the window up a line at a time.
- fScrollDownWindow -- Located at the bottom of the window,
it is used to scroll down a single line at a time.
- scrollbar -- this is the window which does the variable scrolling.
...
CFLAGS= ${CCF} 
LDFLAGS= -L${LIB} -lspad ${LDF} -lX11 -lXpm
