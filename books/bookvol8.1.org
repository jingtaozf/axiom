...
axes(v3d,"off")
#+end_src
From \verb|en.wikipedia.org/wiki/Klein_bottle|. The ``figure 8'' immersion
...
axes(v3d,"off")
#+end_src
The smallest member of the {\sl postine minute} (tiny pasta) family,
{\sl acini de pepe} (peppercorns) are most suited to consommes (clear
...
#+end_src
These shell-like {\sl ravioli} from Piedmont, northern italy, are fashioned
...
zoom(vp,1.1,1.1,1.1)
#+end_src
The diminutive {\sl onellini} (small rings) are part of the extended
{\sl postine minute} (tiny pasta) clan. Their thickness varies between
...
may also be found served in a {\sl timballo} (baked pasta dish).
...
zoom(vp,2.0,2.0,2.0)
#+end_src
{\sl Bucatini} (pierced) pasta is commonly served as a {\sl
...
#+end_src
A spiral-shaped example from the {\sl pasta corta} (short pasta)
family, and of rather uncertain pedigree, {\sl buccoli} are suitable
...
#+end_src
Literally ``little squids'', {\sl calamaretti} are small ring-shaped
pasta cooked as {\sl pastasciutta} (pasta boiled and drained) then
dished up with a tomato-, egg-, or cheese-based sauce. Their shape
means that {\sl calamaretti} hold both chunky and thin sauces equally
...
zoom(v3d,2.0,2.0,2.0)
#+end_src
Made with wheat flour, eggs, and olive oil, {\sl cannelloni} (big
...
{\sl cannelloni} are covered with a creamy {besciamella} sauce, a
...
#+end_src
Known as ``little tubes'', {\sl cannolicchi} exist both in a {\sl
rigati} (grooved) and {\sl lisci} (smooth) form. These hollow {\sl
pasta corta} (short pasta) come in various diameters and are often
served with seafood. {\sl Cannolicchi} hail from Campania in southern Italy.
...
casarecce} (home-made) are best cooked as {\sl postasciutta} (pasta
boiled, drained and dished up with a sauce). Often {\sl casarecce} are
served with a classic {\sl ragu} and topped with a sprinkle of pepper
...
paguri} (hermit crabs) but renamed {\sl castellane} (castle
dwellers). The sturdy form and rich nutty taste of {\sl castellane}
...
zoom(v3d,3.0,3.0,3.0)
#+end_src
Perfect with chunky sauces made from lamb or pork, {\sl cavatappi}
(corkscrews) are 36 mm-long, hollow helicoidal tubes. As well as an
accompaniment to creamy sauces, such as {\sl boscaiola} (woodsman's)
...
twisted {\sl casareccia}, {\sl cavatelli} can be served {\sl alla
...
rotate(v3d,90,180)
#+end_src
This pasta - available in both {\sl rigoti} (grooved) and {\sl lisci}
(smooth) forms - is typically cooked in broth or served in {\sl rogu
alla bolognese}, though {\sl chifferi rigati} also make an excellent
...
similar in shape to {\sl fusilloni} (a large {\sl fusilli}) but is
substantially longer. {\sl Colonne Pompeii} (columns of Pompeii) are
...
zoom(v3d,1.5,1.5,1.5)
#+end_src
Shaped like their namesake, {\sl conchiglie} (shells) exist in both
{\sl rigate} (grooved) and {\sl lisce} (smooth) forms. Suited to light
tomato sauces, ricotta cheese or {\sl pesto genovese}, {\sl
...
as {\sl minestrone}. Alternatively, these shells can accompany a meat-
...
zoom(v3d,1.5,1.5,1.5)
#+end_src
The shape of {\sl conchiglioni rigate} (large ribbed shells) is ideal
...
a gratin. {\sl Conchiglioni rigati} are often served in the
Italian-American dish {\sl pasta primavera} (pasta in spring sauce)
...
axes(v3d,"off")
#+end_src
This famous {\sl pasta lunga} (long pasta) is made with durum-wheat
flour, water, and in the case of {\sl fettuccine alluovo}, eggs
ideally within days of laying. {\sl Fettuccine} (little ribbons) hail
from the Lazio region. Popular in many dishes, they are an ideal
accompaniment to {\sl Alfredo} sauce, a rich mix of cream, parmesan,
garlic, and parsley.
