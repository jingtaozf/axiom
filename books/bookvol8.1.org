...
polynomials in $x$, both having only integer powers.
  ...
  1. [2.2] *integral* -- Certain continuous functions are not expressible
...
Fresnel integrals, and the error function.
  1. [2.3] *transcendental* -- The transcendental curves cannot be
...
logarithmic ($\log(x)$), or trigonometric ($\sin(x)$, $\cos(x)$). 
  ...
  1. [2.4] *piecewise continuous* -- Other curves, except at a few
