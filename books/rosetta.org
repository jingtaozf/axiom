...
\evensidemargin 0pt
%
#+LATEX: \renewcommand{\textfraction}{.1}
#+LATEX: \renewcommand{\floatpagefraction}{.75}
%
\catcode`@=11
...
%
\def\nth#1{$#1^\ifcase#1 th\or st\or nd\or rd\else th\fi$}
%
% Abbreviations
%
...
\begin{tabular}{l|ll}
& \m{2}Unix/Microsoft user initialization file \\
\hline
\Axiom       & \~{}/axiom.input          & \\
\GAP         & \~{}/.gaprc               & GAP.RC \\
\Gmp         &                           & \\
...
\begin{tabular}{l|ll}
& \h{Describe /keyword/} & \h{Find keywords containing /pattern/} \\
\hline
...
\DoCon       &                        &                        & \\
\GAP         & Read("file");          & time; \h{(also see =Runtime();=)}&quit;\\
\Gmp         &                        &                        & \\
...
\begin{tabular}{l|lll}
& \m{2}Prepend/append an element to a list & \h{Append two lists} \\
\hline
...
\footnotetext{See the definition of =matrix= above.}
...
\begin{tabular}{l|ll}
& \h{Simple loop and Block} & \h{Generate the list $[1, 2, ..., n]$} \\
\hline
...
object, for example =x:=X(Rationals,"x");= or {\tt
f:=FreeGroup(2);x:=f.1;}.}
...
\footnotetext=procedure xapply(f, lst); lisp(f . cdr(lst))\$=
\addtocounter{footnote}{-1}%
...
\Derive      &            &                & /various/\fnm       \\
\DoCon       &            &                & \\
\GAP         &\m{3}There are no formal unevaluated expressions\\
\Gmp         &            &                & \\
...
\addtocounter{footnote}{-1}%
\footnotetext{=TERMS=, =FACTORS=, =NUMERATOR=, =LHS=, etc.}
...
\begin{tabular}{l|l}
& \h{Import two space separated columns of integers from =file=} \\
\hline
\Axiom       & \\
...
\begin{tabular}{l|l}
& \h{Export two space separated columns of integers to =file=\fnm} \\
\hline
\Axiom       & )set output algebra "file" \q (creates file.spout)       \\
             & for i in 1..n repeat output( \_                                \\
             & \q concat([string(xy(i, 1)), " ", string(xy(i, 2))]) )         \\
             & )set output algebra console                                    \\
\Derive      & xy  [Transfer Print Expressions File]\q(creates file.prt)\\
\DoCon       & \\
...
\footnotetext{Some editing of =file= will be necessary for all systems but
...
\Yacas       &        &      &     &                &         & \\
\end{tabular} \\[10pt]
\footnotetext{=ER= represents special cyclotomic numbers and is not a
root function.}
\addtocounter{footnote}{-1}%
...
\footnotetext{This is an option for =Integrate=.}
...
\begin{tabular}{l|ll}
& \m{2}Decrease/increase angles in trigonometric functions \\
\hline
