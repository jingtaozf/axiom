...
\begin{itemize}
\item[(1)] the variable x from the domain =Variable= to the domain 
=Polynomial Integer=;
\item[(2)] the number 1 from the domain =PositiveInteger= to the domain 
=Polynomial Integer=, passing via =Integer=;
\item[(3)] the result $x + 1$ from the domain =Polynomial Integer= 
...
\item /coercion to OutputForm;/
\item /coercion from each $D_i$ to the union domain;/
...
\begin{itemize}
\item {\sl all boolean predicates end in a *?* , as in =odd?=, 
...
\begin{itemize}
\item *Polynomial* is the type Axiom assigns by default to polynomial-like
...
\item *PolynomialCategory* is the category of the result of 
...
\item *PolynomialRing* is used in the implementation of =Polynomial=
(via\\
...
\item *FreeModule* is used in the implementation of =PolynomialRing=,
...
\begin{itemize}
\item[(i)] There is a default definition of exponentiation in =Monoid=,
...
\begin{enumerate}
\item a bounded coercion path $D\Rightarrow...\Rightarrow D^\prime$
