...
The =axiom= command is a shell script that collects the 
command line options for the =sman= process, sets some shell
variables, and then invokes =sman=. 
...
* The axiom Command
The =axiom= command starts everything for Axiom. The options
for the =axiom= command are:
#+begin_example
axiom
...
#+begin_example
  [-ht    |-noht]      whether to use HyperDoc
#+end_example
=Hyperdoc= is the documentation tool for Axiom. The
=-ht= option, enabled by default, will start this tool.
...
#+begin_example
  [-gr    |-nogr]      whether to use Graphics
#+end_example
The =graphics= subsystem is enabled using the 
=-gr= option, enabled by default. Graphics will appear as a
...
#+begin_example
  [-clef  |-noclef]    whether to use Clef
#+end_example
The =clef= (Command Line Edit Facility) allows for command completion.
The list of command completion strings is in the last chapter of this
document. If =clef=, enabled by default, is running then you can type:
#+begin_example
 x:Dena<tab>
#+end_example
...
The =clef= program also allows command line editing. The commands are
...
further information on the =clef= command.

*** [-noiw $\vert$ -iw]
#+begin_example
  [-noiw  |-iw]        start in interpreter in a separate window
#+end_example
The =iw= option, disabled by default, will start a second
...
=)frame= command. For instance, if you type
#+begin_example
 axiom -iw
#+end_example
...
#+begin_example
 x:=3
#+end_example
in the second window will set the variable $x$ in the frame =frame1=.
...
#+begin_example
   ...
   (1)  x
                             Type: Variable x
#+end_example
since the first window is in =frame0= and the variable $x$ is
defined in =frame1=.
...
information on the =frame= command.
...
used in combination with the =-iw= option:
#+begin_example
 axiom -noihere -iw &
#+end_example
...
#+begin_example
  [-go  |-nogo]        whether to start system
#+end_example
uses the =-go= option, enabled by default, controls whether
the system starts from the command line. If the =-nogo=
...
line options to =sman= will be. For instance:
#+begin_example
 axiom -nogo -iw
#+end_example
...
#+begin_example
  [-ws wsname]         use named workspace
#+end_example
In the =-nogo= command above you can see that the default
workspace name is 
#+begin_example
...
of Axiom is created in the =obj/linux/bin= directory. The
=debugsys= image uses interpreted lisp code rather than
...
#+end_src
The =MALLOCTYPE= shell variable is an =IBM= =AIX=
...
memory management routine that does not zero =malloc= memory
...
The =ciao= script for immediate exit:
#+NAME: axiomcmd
#+begin_src common-lisp :noweb yes
ciao() {
	echo "Goodbye."
...
#+end_src
The =needsubopt= script which is used to issue an error message
...
the environment and command line. The term =spad= is an
historical shortened version of the name =scratchpad=,
the original name of the =Axiom= system.
#+NAME: axiomcmd
#+begin_src common-lisp :noweb yes
...
#+begin_src common-lisp :noweb yes
		...
		;;
#+end_src
We handle the various [=-option= $\vert$ =-nooption=] cases
#+NAME: axiomcmd
#+begin_src common-lisp :noweb yes
...
*** sman\_catch\_signals
#+NAME: sman.smancatchsignals
#+begin_src common-lisp :noweb yes
static void
...
#+end_src
*** exec\_command\_env
Note that the next-to-last argument of =execle= must be an
explicit NULL pointer. The previous naked 0 value was not correct.
#+NAME: sman.execcommandenv
#+begin_src common-lisp :noweb yes
static void
...
#+NAME: sman.startthelocalspadclient
#+begin_src common-lisp :noweb yes
static void
...
#+NAME: sman.startthesessionmanager
#+begin_src common-lisp :noweb yes
static void
...
#+end_src
*** start\_the\_hypertex
#+NAME: sman.startthehypertex
#+begin_src common-lisp :noweb yes
static void
...
#+NAME: sman.printspadprocesslist
#+begin_src common-lisp :noweb yes
#if 0
static void
...
\chapter{The =viewman= program}
#+NAME: the viewman command line
#+begin_src common-lisp :noweb yes
char *GraphicsProgram = "$AXIOM/lib/viewman";
#+end_src

\chapter{The =hypertex= program}
#+NAME: the hypertex command line
#+begin_src common-lisp :noweb yes
char *HypertexProgram = "$AXIOM/bin/hypertex -s";
#+end_src
\chapter{The =clef= program}
#+NAME: the clef command line
#+begin_src common-lisp :noweb yes
char *ClefProgram = "$AXIOM/bin/clef -f $AXIOM/lib/command.list -e ";
#+end_src
\chapter{The =session= program}
#+NAME: the session manager command line
#+begin_src common-lisp :noweb yes
...
#+NAME: ses.readSpadServercommand
#+begin_src common-lisp :noweb yes
static void 
...
#+NAME: ses.readmenuclientcommand
#+begin_src common-lisp :noweb yes
static void
...
#+NAME: ses.acceptsessionconnection
#+begin_src common-lisp :noweb yes
static int
...
#+end_src
\chapter{The =spadclient= program}
#+NAME: the spadclient command line
#+begin_src common-lisp :noweb yes
