...
\item search for chunk =gcl-2.6.8pre7.h.linux.defs.patch=
...
\item Change the GCLVERSION to point at the new sources
\begin{enumerate}
\item emacs =$AXHOME/silver/Makefile.pamphlet=
\item search for =#GCLVERSION=, a Makefile comment line
...
\begin{itemize}
\item *ORDRING-* the implementing file
\item *sign* the function name
\item *SI* returns SingleInteger (an old domain name)
\item *3* the third function in the file
...
\item Create the =\section= tag. This tag should contain the exact text of
