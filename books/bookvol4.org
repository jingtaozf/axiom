...
2. Update the patches to the new version
  1. find the previous patches
    ...
    #+end_example
  2. for each patch do ( Step 3 ; Step 4 )
3. create diff -Naur patches to the gcl sources
  ...
  2. move to the subdirectory containing the file
    #+begin_example
    cd /tmp/gcl-Version_2_6_10/gcl-2.6.10/h
    #+end_example
  3. edit the 'linux.defs' file to create the proper patch
  4. save the changed file as linux.defs.tpd
  5. in a shell, create a diff -Naur patch by
    #+begin_example
    diff -Naur linux.defs linux.defs.tpd >gcl-2.6.10.h.linux.defs.patch
    #+end_example
  6. move it to the zips directory
    #+begin_example
...
4. update lsp/Makefile.pamphlet to apply the patch at build
  ...
  2. search for chunk =gcl-2.6.8pre7.h.linux.defs.patch=
  3. copy the chunk and name the new chunk
    =gcl-2.6.10.h.linux.defs.patch=
5. add a new chunk to lsp/Makefile.pamphlet to build gcl-2.6.10
  1. find the subsection ``The GCL-2.6.8pre7 stanza''
  2. make a copy named ``The GCL-2.6.10 stanza''
...
6. Change the GCLVERSION to point at the new sources
  ...
  2. search for =#GCLVERSION=, a Makefile comment line
  3. the last line is uncommented. Assume it reads GCLVERSION=gcl-2.6.8pre7\\
    gcl-2.6.8pre7 was is the name of the current version we are
    replacing. We will use this name in the next step
  4. put a # in front of the GCLVERSION variable to comment it out\\
...
-- And you can format things *bold*
#+end_example
...
3. copy the shell script to the file \$AXIOM/bin/AXIOMsys so it will
...
3. Create a new section. The easiest way to do this is to copy another
  section and change the names. In any case, your new section will need
4. Create a dividiing line consisting of all \% signs.
  Note that this character is the comment character for TeX.
5. Create the =\section= tag. This tag should contain the exact text of
  the )abbrev line from your algebra code. It reads:
  =)section{package INTERGB InterfaceGroebnerPackage}=
6. Create a regression test chunk. This chunk derives its name from the
  ...
  #+end_example
7. Create a help text chunk. This chunk dervies its name from the
