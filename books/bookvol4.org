...
1. X and Y are known to be of mode Integer, but their values are
...
1. Nothing is known about X and Y. In this case, we have no idea
...
1. Update the patches to the new version
1. create diff -Naur patches to the gcl sources
1. update lsp/Makefile.pamphlet to apply the patch at build
1. add a new chunk to lsp/Makefile.pamphlet to build gcl-2.6.10
1. Change the GCLVERSION to point at the new sources
1. change the Makefile to match Makefile.pamphlet
...
#+begin_example
      mv gcl gcl-2.6.10
#+end_example
  1. rename gcl to use our naming convention
#+begin_example
      tar -zcf gcl-2.6.10.tgz gcl-2.6.10
#+end_example
  1. We move the original, renamed, retarred file to the zip directory
#+begin_example
      mv gcl-2.6.10.tgz $AXHOME/silver/zips
#+end_example      
  1. We have to make sure to include the new file in the git commit
#+begin_example
     cd $AXHOME/silver
#+end_example      
  1. Tell git we care about the file
#+begin_example
     git add zips/gcl-2.6.10.tgz
#+end_example
1. Update the patches to the new version
  1. find the previous patches
#+begin_example
     ls $AXHOME/silver/zips/gcl-2.6.8pre7*patch
#+end_example
  1. for each patch do ( Step 3 ; Step 4 )
1. create diff -Naur patches to the gcl sources
  1. assume we are looking at gcl-2.6.8pre7.h.linux.defs.patch\\
...
#+end_example
  ...
  1. move to the subdirectory containing the file
#+begin_example
      cd /tmp/gcl-Version_2_6_10/gcl-2.6.10/h
#+end_example
  1. edit the 'linux.defs' file to create the proper patch
  1. save the changed file as linux.defs.tpd
  1. in a shell, create a diff -Naur patch by
#+begin_example
      diff -Naur linux.defs linux.defs.tpd >gcl-2.6.10.h.linux.defs.patch
#+end_example
  1. move it to the zips directory
#+begin_example
...
#+end_example
1. update lsp/Makefile.pamphlet to apply the patch at build
  1. edit lsp/Makefile.pamphlet
  1. search for chunk =gcl-2.6.8pre7.h.linux.defs.patch=
  1. copy the chunk and name the new chunk
=gcl-2.6.10.h.linux.defs.patch=
1. add a new chunk to lsp/Makefile.pamphlet to build gcl-2.6.10
  1. find the subsection ``The GCL-2.6.8pre7 stanza''
  1. make a copy named ``The GCL-2.6.10 stanza''
  1. add the new patches
  1. tell git we care
#+begin_example
      cd $AXHOME/silver
...
#+end_example
1. Change the GCLVERSION to point at the new sources
  1. emacs =$AXHOME/silver/Makefile.pamphlet=
  1. search for =#GCLVERSION=, a Makefile comment line
  1. the last line is uncommented. Assume it reads GCLVERSION=gcl-2.6.8pre7\\
      gcl-2.6.8pre7 was is the name of the current version we are
      replacing. We will use this name in the next step
  1. put a # in front of the GCLVERSION variable to comment it out\\
...
1. cp obj/sys/bin/debugsys \$AXIOM/bin/AXIOMsys\\
...
1. create the shell script shown below
1. copy the shell script to the file \$AXIOM/bin/AXIOMsys so it will
execute in place of the normal Axiom image
1. chmod +x \$AXIOM/bin/AXIOMsys to make the script executable
...
1. Find the right chapter. Chapter ordered by name, not abbreviation
1. Create a new section. The easiest way to do this is to copy another
section and change the names. In any case, your new section will need
1. Create a dividiing line consisting of all \% signs.
Note that this character is the comment character for TeX.
1. Create the =\section= tag. This tag should contain the exact text of
the )abbrev line from your algebra code. It reads:
=)section{package INTERGB InterfaceGroebnerPackage}=
1. Create a regression test chunk. This chunk derives its name from the
...
#+end_example
1. Create a help text chunk. This chunk dervies its name from the
...
1. create the pamphlet file called newbook.pamphlet
1. create a new section in newbook.pamphlet
1. create the chunk name stanza in newbook.pamphlet
1. insert BLAS1.spad into the new stanza
1. create the dotabb stanza in newbook.pamphlet
1. update src/algebra/Makefile.pamphlet to insert BLAS1.spad
1. update src/algebra/Makefile.pamphlet to insert BLAS1 into graph
1. update the dotabb stanza in newbook.pamphlet
1. update src/Makefile.pamphlet to copy the newbook file to src/algebra
1. update \$globalExposureGroupAlist in bookvol5 to add BLAS1 to basic
