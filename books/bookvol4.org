...
value is a /modemap list/\footnote{We imply here that a modemap
...
/subrs/ (i.e. BPIs) are to conventional LISPs: they are the
...
the problem of /polymorphic operators/, i.e. those which
...
We can move up from this to /parameterised data types/ or just
/types/, also known as 'type generators', as defined by ADJ
[cite:@Ehri80a].
...
The previous definitions allow us to define a /domain/
...
/any/ class of domains. While the MODLISP system places no
...
1. X is 2 of mode Integer, Y is 3 of mode Integer. Then the modemap
for PLUS tells us to apply the function IntegerPlus, and the evaluator
does so, returning the answer 5.
2. X and Y are known to be of mode Integer, but their values are
...
3. Nothing is known about X and Y. In this case, we have no idea
...
*** Compilation for Arbitrary Domains
...
/polymorphic functions/, such as a sort function that will sort
...
consider a /data type/ (which corresponds to our /domain/) to
be a (many-sorted) /algebra/ and a list of operations between the
...
and operations:
\[\begin{array}{ll}
=Create=: & () \rightarrow S (=no arguments=),\\
=Push=:   & (I,S) \rightarrow S,\\
=Pop=:    & (S) \rightarrow S\footnote{Here we avoid detailed discussions
...
We now define a /category/ of algebras (i.e. types) to be the
...
\[\begin{array}{ll}
=Create=: & () \rightarrow C_S (=no arguments=),\\
=Push=:   & (C_I,C_S) \rightarrow C_S,\\
=Pop=:    & (C_S) \rightarrow C_S\\
=Top=:    & (C_S) \rightarrow C_I,\\
=Empty=:  & (C_S) \rightarrow Boolean
\end{array}\]
...
*** Appendix A: Description of MODLISP Evaluator
...
is used for an
atomic datatype called /triple/ consisting of an expression X, a
...
*** On Extensions for Partial Compilation
...
*** On Extensions for Partial Declarations
...
NIL\footnote{This initial setting determines a /bottom-up/
...
first to try a /top-down/ evaluation, and then to use a bottom-up
...
*** Appendix B: Sample Compilation by the MODLISP Evaluator
...
** Tedious Maintainer Tasks

*** Maintaining the credits list
...
=Q(\pi)= of the rational numbers. This type allows interactive
users to use the name ='%pi'= without a type both where a numerical
value is expected [ as in =draw(sin x,x=-%pi..%pi)= ] or when the
exact symbolic value is meant.  The interpreter defaults a typeless
=%pi= to HACKPI and then uses the various conversions to cast it
...
=$TMP/trace= but we can watch every detail with the command line:
#+begin_example
  make NOISE=
#+end_example
...
** The runtime structure of Axiom
\begin{center}
[[ps/v4architecture.eps]]\\
*Runtime Structure [cite:@Bake14]*
\end{center}
...
*** Where each output file is created
...
#+end_example
*** A simple integral, expansion 1 interpreter
...
#+end_example
*** A simple integral, expansion 2 integrate
...
1>exit  IntegrationResultRFToFunction.integrate,32 : 
 ...
 ------------     <== Union(Expression Integer, List Expression Integer)
       a
#+end_example
*** A simple integral, expansion 2 internalIntegrate
...
#+begin_example
 1>exit  RationalFunctionIntegration.internalIntegrate,25 : 
  ...
  a        a
#+end_example
*** A simple integral, expansion 3 univariate
...
*** A simple integral, expansion 4 integrate
...
*** A simple integral, expansion 5 monomialIntegrate
...
(35) -> 
#+end_example
*** A simple integral, expansion 6 HermiteIntegrate
...
- set up a local public key:
ssh-keygen -b 1024 -t rsa1
- open a browser
- nagivate to the savannah page that has your personal keys
...
4. Add the latest GCL sources to Axiom
5. Update the patches to the new version
6. create diff -Naur patches to the gcl sources
7. update lsp/Makefile.pamphlet to apply the patch at build
8. add a new chunk to lsp/Makefile.pamphlet to build gcl-2.6.10
9. Change the GCLVERSION to point at the new sources
10. change the Makefile to match Makefile.pamphlet


We assume in the following that =$AXHOME= is the home directory and that
Axiom lives in the silver subdirectory.

In more detail these steps are:

11. Add the lateset GCL sources to Axiom
  1. Download the latest GCL from gnu.org\\
      For these instructions assume the file is 
#+begin_example
      gcl-Version_2_6_10.tar.gz
#+end_example
  2. move the tar file into /tmp\\
      We are going to make changes to the distribution via patches
  3. untar the file
#+begin_example
      tar -zxf gcl-Version_2_6_10.tar.gz
#+end_example
  4. cd to the untarred directory
#+begin_example
      cd gcl-Version_2_6_10
#+end_example
  5. rename the gcl directory\\
...
#+begin_example
      mv gcl gcl-2.6.10
#+end_example
  6. rename gcl to use our naming convention
#+begin_example
      tar -zcf gcl-2.6.10.tgz gcl-2.6.10
#+end_example
  7. We move the original, renamed, retarred file to the zip directory
#+begin_example
      mv gcl-2.6.10.tgz $AXHOME/silver/zips
#+end_example      
  8. We have to make sure to include the new file in the git commit
#+begin_example
     cd $AXHOME/silver
#+end_example      
  9. Tell git we care about the file
#+begin_example
     git add zips/gcl-2.6.10.tgz
#+end_example
2. Update the patches to the new version
  10. find the previous patches
#+begin_example
     ls $AXHOME/silver/zips/gcl-2.6.8pre7*patch
#+end_example
  11. for each patch do ( Step 3 ; Step 4 )
3. create diff -Naur patches to the gcl sources
  12. assume we are looking at gcl-2.6.8pre7.h.linux.defs.patch\\
...
#+end_example
  ...
  13. move to the subdirectory containing the file
#+begin_example
      cd /tmp/gcl-Version_2_6_10/gcl-2.6.10/h
#+end_example
  14. edit the 'linux.defs' file to create the proper patch
  15. save the changed file as linux.defs.tpd
  16. in a shell, create a diff -Naur patch by
#+begin_example
      diff -Naur linux.defs linux.defs.tpd >gcl-2.6.10.h.linux.defs.patch
#+end_example
  17. move it to the zips directory
#+begin_example
...
4. update lsp/Makefile.pamphlet to apply the patch at build
  18. edit lsp/Makefile.pamphlet
  19. search for chunk =gcl-2.6.8pre7.h.linux.defs.patch=
  20. copy the chunk and name the new chunk
=gcl-2.6.10.h.linux.defs.patch=
5. add a new chunk to lsp/Makefile.pamphlet to build gcl-2.6.10
  21. find the subsection ``The GCL-2.6.8pre7 stanza''
  22. make a copy named ``The GCL-2.6.10 stanza''
  23. add the new patches
  24. tell git we care
#+begin_example
      cd $AXHOME/silver
...
6. Change the GCLVERSION to point at the new sources
  25. emacs =$AXHOME/silver/Makefile.pamphlet=
  26. search for =#GCLVERSION=, a Makefile comment line
  27. the last line is uncommented. Assume it reads GCLVERSION=gcl-2.6.8pre7\\
      gcl-2.6.8pre7 was is the name of the current version we are
      replacing. We will use this name in the next step
  28. put a # in front of the GCLVERSION variable to comment it out\\
...
** Axiom internal representations
#+begin_example
...
** Spad to internal function calling
...
- *3* the third function in the file
...
#+end_example
returns the property list on the symbol *Permutation* which is where
...
The =(qrefelt $ 58)= uses the above dance to look up a function
...
*** Adding help documentation in Makefile
...
*** Using help documentation for regression testing
...
*algebra.regress*. It contains a list of regression files and a 
single stanza:
#+begin_example
%.regress: %.input
...
#+end_example
The input Makefile extracts *algebra.regress* and then calls 
make to process this file.

This keeps the regression test list in the algebra Makefile.

*** help documentation as algebra test files
...
12. mv \$AXIOM/bin/AXIOMsys \$AXIOM/bin/AXIOMsys.backup\\
This keeps the failing axiomsys image around for later restoration.
13. cp obj/sys/bin/debugsys \$AXIOM/bin/AXIOMsys\\
...
** Understanding a compiled function
...
** Where are Axiom symbols stored?
...
** Translating individual boot files to common lisp
...
*** The mnt/linux/algebra directory
./algebra
...
** Hand creating the hyperdoc binary
...
general method.
*** Finding Anonymous Function Signatures
...
We see that differentiate is called with =#1=, which is Axiom's
...
=+->= notation. This notation allows you to explicitly specify
type information. In the above code, we would like to replace the
=#1= variable with the =+->= and explicit type information.
...
=$= is a hidden, internal fourth argument to an Axiom three argument
...
=$= and slot number and does a ``quick array reference''. The
...
of =(QREFELT $ 19)= to find the function to call.
...
The call of =(setq $dalymode t)= changes the Axiom top level
...
saves typing =)lisp= in front of every lisp expression. Be sure
to do a =(setq $dalymode nil)= when you are finished.
...
The =(setf dv ...= uses the Lisp internal names for the domains.
...
#+end_example
Note that =#0= is a ``pointer'', in this case to the list '(c)
and =#0#= is a use of that pointer. This is done to make sure that
...
From this we can see a reference to =FS-;differentiate;SSS;99=
...
code for =FS-;differentiate;SSS;99= lives in 
...
*** Operating system level I/O trace (strace)
...
15. rename \$AXIOM/bin/AXIOMsys to \$AXIOM/bin/AXIOMsys.bin
16. create the shell script shown below
17. copy the shell script to the file \$AXIOM/bin/AXIOMsys so it will
execute in place of the normal Axiom image
18. chmod +x \$AXIOM/bin/AXIOMsys to make the script executable
19. start axiom normally
...
** How to make graphs in algebra books
...
** Adding or Editing pages in Hyperdoc
...
- remove all tabs. Spad is a language that assigns meaning to
indentation and tabs are not going to survive the build process intact.
- try to stay within 80 characters. Spad code is printed in the books
so it should try to limit line lengths everywhere.
- change ``)abbreviation'' to ``)abbrev''. The Makefile will search
for the abbrev line and expect this exact text. 
\begin{verbatim}
)abbrev package INTERGB InterfaceGroebnerPackage
\end{verbatim}
- make sure there is only a single space between the items in the
abbrev line. The Makefile assumes this.
- Add the comment header block. The author information is used to
...
21. Find the right chapter. Chapter ordered by name, not abbreviation
22. Create a new section. The easiest way to do this is to copy another
section and change the names. In any case, your new section will need
23. Create a dividiing line consisting of all \% signs.
Note that this character is the comment character for TeX.
24. Create the =\section= tag. This tag should contain the exact text of
the )abbrev line from your algebra code. It reads:
=)section{package INTERGB InterfaceGroebnerPackage}=
25. Create a regression test chunk. This chunk derives its name from the
...
#+end_example
26. Create a help text chunk. This chunk dervies its name from the
...
=$AXIOM/books/ps/v104interfacegroebnerpackage.ps=
...
#+end_example
to the variable =$globalExposureGroupAlist= under the ``basic''
sublist. 
...
*** Creating a stand-alone pamphlet file
...
27. write the algebra code
28. create the pamphlet file called newbook.pamphlet
29. create a new section in newbook.pamphlet
30. create the chunk name stanza in newbook.pamphlet
31. insert BLAS1.spad into the new stanza
32. create the dotabb stanza in newbook.pamphlet
33. update src/algebra/Makefile.pamphlet to insert BLAS1.spad
34. update src/algebra/Makefile.pamphlet to insert BLAS1 into graph
35. update the dotabb stanza in newbook.pamphlet
36. update src/Makefile.pamphlet to copy the newbook file to src/algebra
37. update \$globalExposureGroupAlist in bookvol5 to add BLAS1 to basic
